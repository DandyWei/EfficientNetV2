...
'''
equeeze(swish) and expand(sigmoid)
use bias if not BatchNormalization
'''
...
class MBConv(layers.Layer):
    def __init__(self,
        ...
        '''
        順序
        先過11 filters 擴展 channel 到 expanded_c數量
        BN(所以前面不使用BIAS)，超參數跟論文GITHUB一樣
        激活函數SWISH

        DWConv make channel to N times of last layer, no bias
        BN
        activation funtion swish
        ...
        # SE
        '''
        sqeeze into num_reduced_filters
        then expand to expanded_c
        the channel is same as input => expanded_c
        '''
        ...
        '''
        downgrade channels to same size as input channels
        then also no bias for Bn
        ...
        '''
        drop drop_path
        drop the path, if there is a short cut.
        drop rate will increase when the framework is deeper
        '''
...
class FusedMBConv(layers.Layer):
    def __init__(self,
        ...
        '''
        conpare with MBConv:
            1.filter size is 3 x 3 in first layer
            2,
        '''
        if expand_ratio != 1:
            self.conv2d = layers.Conv2D(
...
'''
outputl layer
fintune classnumber to train our models
'''
